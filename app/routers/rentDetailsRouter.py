...
def get_all_rent_details(db: Session = Depends(database.get_db)):
    return rentDetailsService.get_all_rent_details(db)


@router.post('/', status_code=status.HTTP_201_CREATED)
def create_rent_details(request: RentDetailsCreateRequest, db: Session = Depends(get_db)):
    return rentDetailsService.create_rent_details(request, db)


@router.delete('/{id}', status_code=status.HTTP_202_ACCEPTED)
def delete_rent_details(id: UUID, db: Session = Depends(get_db)):
    return rentDetailsService.delete_rent_details(id, db)


@router.put('/{id}', status_code=status.HTTP_202_ACCEPTED)
def update_rent_details(id: UUID, request: RentDetailsCreateRequest, db: Session = Depends(get_db)):
...
def get_rent_details_by_id(id: UUID, db: Session = Depends(get_db)):
